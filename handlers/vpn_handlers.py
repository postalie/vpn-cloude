...
async def process_purchase(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Формируем payload для шифрования
    domain_clean = BASE_URL.replace("https://", "").replace("http://", "")
    payload = f"add/{domain_clean}/{user_id}/{sub_uuid}"
    encrypted = base64.urlsafe_b64encode(payload.encode()).decode().replace("=", "")
    
    # Ссылка GitHub
    gh_link = f"{GITHUB_PAGE_URL}/{encrypted}"

    # Используем новую клавиатуру с GH ссылкой
    from keyboards import get_active_sub_kb, get_devices_kb
    kb = get_active_sub_kb(gh_link)
